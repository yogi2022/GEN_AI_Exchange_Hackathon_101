...
async def submit_application(application: StartupApplication):
    """Submit a new startup application for evaluation"""
...
async def schedule_interview(
    ...
    scheduled_time = preferred_times[0] if preferred_times else "2024-12-01T10:00:00Z"
    
    return {
        "application_id": application_id,
        "scheduled_time": scheduled_time,
        "meeting_link": f"https://meet.google.com/demo-{application_id[:8]}",
        "status": "scheduled"
    }
...
@app.post("/api/customize-weights")
async def customize_evaluation_weights(
    preferences: InvestorPreferences,
    user: dict = Depends(get_current_user)
):
    """Customize evaluation weights based on investor preferences"""
...
async def evaluate_application(application: StartupApplication):
    """Background task to evaluate startup application"""
    try:
...
print("  - Customizable evaluation weights")